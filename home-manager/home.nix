    ...
    initLua = ''
      require("yatline"):setup({
        ...
        status_line = {
          ...
          right = {
            section_a = {
              {type = "string", custom = false, name = "disk_space"},
            },
            section_b = {
      ...
      -- Register custom disk space component as a string-based addon
      function Yatline.string.get:disk_space()
        local cwd = cx.active.current.cwd
        local handle = io.popen("df -h '" .. tostring(cwd) .. "' 2>/dev/null | tail -1 | awk '{print $4, $5}'")
